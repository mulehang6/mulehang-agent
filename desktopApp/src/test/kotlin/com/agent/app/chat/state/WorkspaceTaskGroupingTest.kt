...
class WorkspaceTaskGroupingTest {
    ...
    /** 未关联历史保留在任务状态中，但不得生成侧栏工作区分组。 */
    @Test
    fun `should hide unlinked history from workspace groups while retaining tasks`() {
        val ui = ChatWindowUiState(
            tasks = listOf(
                ...
                ChatConversationUiState(
                    ...
                    title = "保留的历史",
                    workspacePath = "",
                    items = listOf(ChatMessageItem(ChatMessage(ChatRole.User, "历史内容"))),
                ),
            ),
            activeTaskId = "linked",
        )
        ...
        assertTrue(ui.tasks.any { it.id == "unlinked" })
        assertEquals(listOf("D:\\work\\alpha"), ui.workspaceGroups.map { it.workspacePath })
        assertEquals(listOf("D:\\work\\alpha"), ui.workspaceTaskSections.map { it.workspacePath })
    }
