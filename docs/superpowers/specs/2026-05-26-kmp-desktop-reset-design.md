...
当前仓库旧主线建立在 `runtime-first + Bun/OpenTUI + vendor/kilocode` 的结构之上。该结构参考了 Kilo 的整体思路，但对当前项目阶段来说复杂度过高，且大量 AI 生成代码已经明显拉高了维护成本。

本次调整不再沿用旧主线。新的目标是把仓库重置为一个面向 Windows 桌面应用的 Kotlin 项目，以 Koog 1.0.0、Kotlin Multiplatform 和 Compose Multiplatform Desktop 为核心技术栈。

这里的清理目标特指旧主线对外置仓库的主线依赖关系。这份设计不把 `vendor/` 目录本身定义为永久禁区；如果后续因为参考、镜像或并行实验再次引入 `vendor/kilocode` 或其他独立项目，只要它们不重新成为仓库主线依赖，就不违背本设计。

## 目标

本轮重置的目标如下：

1. 删除旧 `runtime/`、`cli/` 及围绕旧主线建立的主线文档体系。
...
上面的主线结构不要求 `vendor/` 必须不存在；它只说明当前产品主线围绕 `desktopApp/` 与 `shared/` 两个模块展开，而不是建立在任何 `vendor/*` 子模块之上。
...
1. 删除 `runtime/`
2. 删除 `cli/`
3. 删除旧 `runtime-first`、`CLI first`、`ACP second` 文档体系
4. 用新的 KMP Desktop 文档替换当前主线设计说明
