...
class DesktopUiStateStoreTest {
    ...
    fun `should persist normalized appearance preferences`() {
        ...
        DesktopUiStateStore(databasePath).saveAppearancePreferences(
            DesktopAppearancePreferences(126, "Inter", "JetBrains Mono"),
        )

        assertEquals(
            DesktopAppearancePreferences(130, "Inter", "JetBrains Mono"),
            DesktopUiStateStore(databasePath).loadAppearancePreferences(),
        )
    }
    ...
    fun `should merge independent ui preferences`() {
        ...
        val store = DesktopUiStateStore(databasePath)
        ...
        val reopened = DesktopUiStateStore(databasePath)
    ...
    fun `should not migrate or delete legacy ui state`() {
        ...
        val store = DesktopUiStateStore(root.resolve("mulehang.db"))
